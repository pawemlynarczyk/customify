...
// FIX: Poprawne proporcje galerii - większe główne zdjęcie, mniejsze miniaturki
function fixGalleryProportions() {
  const galleryGrid = document.querySelector('.media-gallery__grid');
  if (!galleryGrid) return;
  
  console.log('🔧 Naprawiam proporcje galerii...');
  
  // Wymuś nowe wymiary przez JavaScript
  galleryGrid.style.setProperty('grid-template-rows', '400px 120px', 'important');
  galleryGrid.style.setProperty('grid-template-columns', '120px 120px', 'important');
  galleryGrid.style.setProperty('height', '524px', 'important');
  
  // Główne zdjęcie
  const mainImage = galleryGrid.querySelector('li:first-child');
  if (mainImage) {
    mainImage.style.setProperty('height', '400px', 'important');
    mainImage.style.setProperty('width', '100%', 'important');
    mainImage.style.setProperty('grid-row', '1 / 2', 'important');
    mainImage.style.setProperty('grid-column', '1 / 3', 'important');
    
    const mainImg = mainImage.querySelector('img');
    if (mainImg) {
      mainImg.style.setProperty('height', '400px', 'important');
  ...
  // Miniaturki
  const thumbnails = galleryGrid.querySelectorAll('li:nth-child(2), li:nth-child(3)');
  thumbnails.forEach((thumb, index) => {
    thumb.style.setProperty('height', '120px', 'important');
    thumb.style.setProperty('width', '120px', 'important');
    thumb.style.setProperty('aspect-ratio', '1 / 1', 'important');
    thumb.style.setProperty('grid-row', '2 / 3', 'important');
    thumb.style.setProperty('grid-column', index === 0 ? '1 / 2' : '2 / 3', 'important');
  ...
  console.log('✅ Proporcje galerii naprawione!');
}

// Uruchom naprawę galerii po załadowaniu strony
document.addEventListener('DOMContentLoaded', function() {
  // Czekaj aż galeria się załaduje
  setTimeout(fixGalleryProportions, 1500);
  
  // Uruchom ponownie co 2 sekundy żeby nadpisać ewentualne zmiany
  setInterval(fixGalleryProportions, 2000);
});
