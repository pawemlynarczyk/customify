// send-bulk-walentynki.js
// Masowa wysyłka maili walentynkowych po 100 maili na raz
...
// Pliki do zapisu postępu
const PROGRESS_FILE = 'mailing-progress.json';
const RESULTS_FILE = 'mailing-results.json';
...
async function sendBatch(customers, batchNumber, totalBatches) {
  ...
  try {
    // Wyślij całą partię na raz (endpoint obsługuje tablicę i ma własny rate limiting)
    ...
    // Wszystkie emaile z partii jako failed
    const results = {
      sent: [],
      failed: [
...
async function main() {
  console.log('🚀 Masowa wysyłka maili walentynkowych');
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    try {
      ...
      // Przerwa między partiami (oprócz ostatniej)
